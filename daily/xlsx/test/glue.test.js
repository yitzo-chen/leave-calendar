...
function baseSheets() {
  return {
    ...
    "工種機具材料清單": [["類別", "項目名稱", "工項編號", "啟用中"], ["工種", "公司工", "", "TRUE"], ["工種", "模板工", "", "TRUE"], ["工種", "舊工種", "", "FALSE"], ["材料", "砂(m³)", "", "TRUE"]],
    "日報頭": [HDR(), ["2026-09-18", "晴", "施工", "第一天", "", "", "王", "c", "t", "甲", "甲", "乙", "乙"], ["2026-09-20", "雨", "施工", "第三天", "", "", "王", "c", "t", "甲", "甲", "乙", "乙"]],
    "日報記錄": [["日期", "項目名稱", "上午", "下午", "clientId", "更新時間"],
      ["2026-09-18", "公司工", 3, 2, "c", "t"], ["2026-09-18", "舊工種", 1, 1, "c", "t"], ["2026-09-18", "砂(m³)", 5, 0, "c", "t"], ["2026-09-20", "公司工", 4, 4, "c", "t"]],
    "本工出勤": [["日期", "人員名稱", "上午", "下午", "上午加班", "下午加班", "加班原因", "clientId", "更新時間"],
      ["2026-09-18", "陳一", "V", "V", 0, 0, "", "c", "t"], ["2026-09-20", "林二", "V", "", 1, 0, "材料進場", "c", "t"]],
  };
}
...
(async () => {
  ...
  const env = makeGlueEnv({ sheetsData: baseSheets(), driveFiles: { "日報範本.xlsx": TEMPLATE_BYTES } });
  let r1;
  T("G2", "首次更新 2026-09-20：建立檔案；只有該日分頁（其他日期未指定不重建）", () => {
    r1 = env.call("xlsxUpdateForDate('2026-09-20')");
  ...
  T("G5", "更新較早的 9/18：同一檔案（用 REST 覆蓋，ID 不變）、兩個分頁、9/20 累計不變", () => {
    fileId = F(env).id;
    const r = env.call("xlsxUpdateForDate('2026-09-18')");
    const wb = sheetVals(F(env).bytes);
  ...
  T("G9", "xlsxRebuildAll：依日報頭全部日期重建", () => { const r = env.call("xlsxRebuildAll()"); return eq([r.full, r.sheets], [true, 2]); });
  T("G10", "輸出檔被移到垃圾桶：改依名稱找不到→建立新檔並更新記錄的 ID", () => {
    F(env).trashed = true;
    const r = env.call("xlsxUpdateForDate('2026-09-20')");
    return eq([r.ok, F(env).id !== fileId, env.state.props.XLSX_FILE_ID === F(env).id], [true, true, true]);
  });
  T("G11", "找不到範本檔：丟出清楚的錯誤訊息", () => {
    const e2 = makeGlueEnv({ sheetsData: baseSheets(), driveFiles: {} });
    try { e2.call("xlsxUpdateForDate('2026-09-20')"); return "應該要丟錯"; } catch (e) { return /找不到範本檔/.test(String(e.message)) || String(e.message); }
  });
  T("G12", "範本結構不對：丟出錯誤，且不動既有輸出檔", () => {
    ...
    try { e2.call("xlsxUpdateForDate('2026-09-20')"); return "應該要丟錯"; } catch (e) { return /範本檔結構不符/.test(String(e.message)) || String(e.message); }
  });
  T("G13", "REST 更新失敗（HTTP 404）：丟出錯誤", () => {
    const e2 = makeGlueEnv({ sheetsData: baseSheets(), driveFiles: { "日報範本.xlsx": TEMPLATE_BYTES } });
    e2.call("xlsxUpdateForDate('2026-09-20')");
  ...
  last.call("xlsxRebuildAll()");
  const out = Object.values(last.state.files).find((f) => f.name === "施工日報彙整.xlsx").bytes;
